...
public class FlammableRotatedPillarBlock extends RotatedPillarBlock {
    ...
    public BlockState getToolModifiedState(BlockState state, Level world, BlockPos pos, Player player,
        ...
        if(stack.getItem() instanceof AxeItem) {
            if(state.is(blockinit.EXAMPLE_LOG.get())) {
                return blockinit.STRIPPED_EXAMPLE_LOG.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
            if(state.is(blockinit.EXAMPLE_WOOD.get())) {
                return blockinit.STRIPPED_EXAMPLE_WOOD.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
        }
